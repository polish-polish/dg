                                 ...
                                 std::set<LLVMNode *>& nodes)
{
    ...
    for (auto& crit : criteria) {
        ...
        fileCritea tmpCritea= fileCritea();
        ...
        if (isNumber(parts[1])) {
            ...
            if (line > 0){
                ...
                llvm::errs() << "#CRITERIA# file: " << parts[0] << "line: " << parts[1] << ",var:" << tmpCritea.varname <<"\n";
            }
    ...
    #endif 
            {
                for (fileCritea & c : parsedCrit) {
                    ...
                    if(name1.compare(name2)!=0)
                        continue;
                    if(!llvm::dyn_cast<llvm::LoadInst>(&I) && !llvm::dyn_cast<llvm::StoreInst>(&I))
                        continue;
                    if (usesTheVariable(dg, I, c.varname)||!c.varname.compare("NULL")) {
