...
static inline bool isTheVar(const llvm::Value *val,
...
static bool usesTheVariable(const llvm::Instruction& I,
                            const std::string& var,
                            bool isglobal = false,
...
static bool funHasAddrTaken(const llvm::Function *fun) {
    ...
    for (auto use_it = fun->use_begin(), use_end = fun->use_end();
        ...
        // FIXME: the function can be in a cast instruction that is just used in call,
        //   we can detect that
        } else {
            llvm::errs() << "Unhandled function use: " << *user  << "\n";
            return true;
...
static bool instIsCallOf(const llvm::Instruction& I,
...
static bool instMatchesCrit(const llvm::Instruction& I,
...
static void getCriteriaInstructions(llvm::Module& M,
    ...
    if (constructed_only) {
        DBG(llvm-slicer, "Checking constructed functions for slicing criteria");
...
getSlicingCriteriaInstructions(llvm::Module& M,
    ...
    for (const auto& crit : criteria) {
        ...
        if (!primsec[0].empty()) {
            getCriteriaInstructions(M, pta, primsec[0], SC.primary, constructed_only);
        }
        ...
        if ((!SC.primary.empty() || ssctoall) && primsec.size() > 1) {
            getCriteriaInstructions(M, pta, primsec[1], SC.secondary, constructed_only);
...
bool getSlicingCriteriaNodes(LLVMDependenceGraph& dg,
    ...
    auto crits = getSlicingCriteriaInstructions(*dg.getModule(),
                                                slicingCriteria,
...
getSlicingCriteriaValues(llvm::Module& M,
                         const std::string& slicingCriteria,
                         const std::string& legacySlicingCriteria,
    ...
    if (legacySlicingCriteria != "") {
        ...
        if (parts.size() == 2) {
            if (legacySecondaryCriteria != "")  {
                criteria += parts[0] + "#" + parts[1] + "|" + legacySecondaryCriteria + "()";
            } else {
                criteria += parts[0] + "#" + parts[1];
            }
        } else if (parts.size() == 1) {
            if (legacySecondaryCriteria != "")  {
                criteria += legacySlicingCriteria + "()|" + legacySecondaryCriteria + "()";
            } else {
    ...
    auto C = getSlicingCriteriaInstructions(M, criteria, criteria_are_next_instr,
                                            /*pta = */ nullptr, /* constructed only */ false);
    for (auto& critset : C) {
        ret.insert(ret.end(), critset.primary.begin(), critset.primary.end());
        ret.insert(ret.end(), critset.secondary.begin(), critset.secondary.end());
    }
    return ret;
}
