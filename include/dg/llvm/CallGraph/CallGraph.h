...
inline bool callIsCompatible(const llvm::Function *F,
...
class LazyLLVMCallGraph : public CallGraphImpl {
    ...
    dg::HashMap<const llvm::Function *,
                std::vector<const llvm::CallInst *>> _callsOf;
    ...
    // we pass also the call inst to be able to filter out incompatible functions
    FuncVec _getCalledFunctions(const llvm::CallInst *C, const llvm::Value *val) {
        if (_pta) {
            FuncVec ret;
...
public:
    ...
    getCallsOf(const llvm::Function *F) {
        if (funHasAddressTaken(F)) {
...
public:
    ...
    CallGraph(const llvm::Module *m, LLVMPointerAnalysis *pta, bool lazy = true)
        : _impl(lazy ? static_cast<CallGraphImpl*>(new LazyLLVMCallGraph(m, pta)) :
                       static_cast<CallGraphImpl*>(new LLVMPTACallGraphImpl(m, pta))) {}
    CallGraph(const llvm::Module *m)
        : _impl(new LazyLLVMCallGraph(m)) {}
